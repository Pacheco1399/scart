<div class="container-fluid">
    <div class="nuevos-cupones">
        <div class="container">
            <div class="row">
                <h2 class="title">¿QUIERES CREAR TUS CUPONES?</h2>
            </div>
            <div class="row">
                <h1 class="subTitle">Ingresa aquí, selecciondo una de las dos opciones. para mas información, haz clic
                    aquí.</h1>
            </div>

            <div class="row">
                <div class="col-md-6">
                    <a class="btn-empresa" type="button" data-bs-toggle="modal" data-bs-target="#modalEmpresa">
                        <h1 class="text">CUPÓN EMPRESA</h1>
                    </a>
                </div>
                <div class="col-md-6">
                    <a class="btn-persona" type="button">
                        <h1 class="text">CUPÓN PERSONA</h1>
                    </a>
                </div>

            </div>
        </div>
    </div>
</div>

<!-- Modal -->
<div class="modal fade" id="modalEmpresa" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
     aria-labelledby="modalEmpresaLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-body">
                <div class="row">
                    <h2 class="" id="modalEmpresaLabel">¿Quires crear tus cupones de empresa</h2>
                </div>
                <div class="row">
                    <h1>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Dolor mauris mauris sit.</h1>
                </div>
                <div class="row">
                    <div class="col-md-6">
                        <input type="text" placeholder="Nombre facilitador">
                    </div>
                    <div class="col-md-6">
                        <input type="text" placeholder="Cargo del Facilitador">
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-6">
                        <input type="text" placeholder="Nombre del e-commerce">
                    </div>
                    <div class="col-md-6">
                        <input type="text" placeholder="Sitio web">
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-6">
                        <input type="text" placeholder="Correo">
                    </div>
                    <div class="col-md-6">
                        <input type="text" placeholder="Cupón">
                    </div>
                </div>
                <div class="row">
                    <textarea placeholder="Detalle o descripción del cupón">

                    </textarea>
                </div>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                <button type="button" class="btn btn-primary">Understood</button>

            </div>

        </div>
    </div>
</div>

